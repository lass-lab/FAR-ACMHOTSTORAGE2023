...
ColumnFamilyData::ColumnFamilyData(
  ...
  int cause;
  RecalculateWriteStallConditions(mutable_cf_options_,cause);
...
WriteStallCondition ColumnFamilyData::RecalculateWriteStallConditions(
      const MutableCFOptions& mutable_cf_options, int& cause) {
...
void ColumnFamilyData::InstallSuperVersion(
    SuperVersionContext* sv_context, InstrumentedMutex* db_mutex,int& write_stall_cause,int& write_stall_cond) {
  db_mutex->AssertHeld();

  return InstallSuperVersion(sv_context, mutable_cf_options_,write_stall_cause,write_stall_cond);
}

void ColumnFamilyData::InstallSuperVersion(
    SuperVersionContext* sv_context,
    const MutableCFOptions& mutable_cf_options,int& write_stall_cause,int& write_stall_cond) {
  ...
  super_version_->write_stall_condition =
    RecalculateWriteStallConditions(mutable_cf_options,write_stall_cause);
  write_stall_cond=(int)super_version_->write_stall_condition;
