  ...
  JobContext job_context(0, true /*create_superversion*/);
  {
    ...
    if (s.ok()) {
      for (auto cfd : cfds_changed) {
        ...
        int write_stall_cause,write_stall_cond;
        cfd->InstallSuperVersion(&sv_context, &mutex_,write_stall_cause,write_stall_cond);
        GetFileSystem()->WriteStallCheckPoint(write_stall_cause,write_stall_cond);
...
Status DB::OpenAsSecondary(
  ...
  *dbptr = nullptr;
  ...
  int write_stall_cause=0,write_stall_cond=0;
  if (s.ok()) {
    for (auto cfd : *impl->versions_->GetColumnFamilySet()) {
      sv_context.NewSuperVersion();

      cfd->InstallSuperVersion(&sv_context, &impl->mutex_,write_stall_cause,write_stall_cond);
  ...
  if (s.ok()) {
    *dbptr = impl;
    (*dbptr)->GetFileSystem()->WriteStallCheckPoint(write_stall_cause,write_stall_cond);
    for (auto h : *handles) {
