...
class FileSystem : public Customizable {
 public:
  ...
  virtual bool IsZoneDevice(void) { return false; }
  virtual bool PreserveZoneSpace(uint64_t ) { return false; }
  virtual void WriteStallCheckPoint(int,int) { }
...
class FileSystemWrapper : public FileSystem {
 public:
  ...
  void SetDBPtr(DB* ptr){
    target_->SetDBPtr(ptr);
  }
  ...
  size_t ZoneCleaning(bool forced){
    return target_->ZoneCleaning(forced);
  }
