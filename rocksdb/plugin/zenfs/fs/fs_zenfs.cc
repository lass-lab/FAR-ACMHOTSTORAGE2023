...
size_t ZenFS::ZoneCleaning(bool forced){
  ...
  if (migrate_zones_start.size() > 0) {
    ...
    zbd_->AddZCTimeLapse(start, end,
                         migrate_zones_start.size() + all_inval_zone_n, forced);
  }
  zc_lock_.unlock();
...
void ZenFS::ZoneCleaningWorker(bool run_once) {
  ...
  while (run_gc_worker_) {
    usleep(100 * 1000);
    if(free_percent_<ZONE_CLEANING_KICKING_POINT){
      ZoneCleaning(false);
...
void ZenFS::SetResetScheme(uint32_t r,bool f,uint64_t T) {
  ...
  if(gc_worker_!=nullptr){
    if(bg_reset_worker_==nullptr){
      bg_reset_worker_.reset(new std::thread(&ZenFS::BackgroundStatTimeLapse, this));
    }
  }
}
